...
const goToTeam = () => navigateTo('/team')

// Onboarding replay. Deliberately not `resetOnboarding()` — that also clears
// hasSeenWelcome, which would pop the welcome modal again on the next dashboard
// visit. Restarting the tour and un-dismissing the checklist is enough.
const replayTour = () => {
  tourStore.checklistDismissed = false
  tourStore.startTour()
  return navigateTo('/')
}
