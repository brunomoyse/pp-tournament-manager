  ...
  <div class="bg-pp-bg-secondary rounded-2xl p-8 shadow-sm border border-pp-border" style="background-color: #24242a !important;">
    ...
    <div class="space-y-6">
      <div class="grid grid-cols-2 gap-6">
        ...
        <div class="text-center">
          <div class="text-3xl font-bold text-pp-text-primary mb-1">{{ playersRemaining }}</div>
      ...
      <div class="space-y-3">
        <div class="flex justify-between items-center text-sm">
          ...
          <span class="text-pp-text-primary font-medium">{{ activePlayers.length }}</span>
        </div>
        <div class="flex justify-between items-center text-sm">
          <span class="text-white/60">Eliminated</span>
          <span class="text-pp-text-primary font-medium">{{ totalRegistered - playersRemaining }}</span>
        </div>
      </div>
      ...
      <div class="space-y-2">
        <div class="flex justify-between items-center text-xs">
          ...
          <span class="text-white/60">{{ Math.round((totalRegistered - playersRemaining) / totalRegistered * 100) || 0 }}%</span>
        </div>
        <div class="w-full bg-pp-bg-primary rounded-full h-2">
          <div 
            class="bg-pp-accent-gold h-2 rounded-full transition-all duration-300"
            :style="{ width: `${Math.round((totalRegistered - playersRemaining) / totalRegistered * 100) || 0}%` }"
          ></div>
        </div>
      </div>
    </div>
...
// Props for player data that comes from GraphQL
...
const activePlayers = computed(() => 
  props.tournamentPlayers.filter((p: TournamentPlayer) => p.registration.status === 'active')
)
</script>